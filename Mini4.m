function varargout = Mini4(varargin)
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Mini4_OpeningFcn, ...
                   'gui_OutputFcn',  @Mini4_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
function Mini4_OpeningFcn(hObject, eventdata, handles, varargin)
...
function varargout = Mini4_OutputFcn(hObject, eventdata, handles) 
h=findobj('tag','Mini')
h=findobj('tag','Mini1')
h=findobj('tag','Mini2')
h=findobj('tag','Mini3')
...
Low1 = getappdata(0,'Low1');
High1 = getappdata(0,'High1');
range1 = (High1 - Low1)/10 ;
Y1=[];
X1=[];
for i=1:10
    Low1 = Low1 + range1;
    X1(i)=Low1;
    Y1(i)=lolipop(handles,Low1);
end
...
doc = Document('Report','docx','Design Report')

X1=getappdata(0,'X1');
Y1=getappdata(0,'Y1');
X2=getappdata(0,'X2');
Y2=getappdata(0,'Y2');
Ds = num2str(getappdata(0,'Ds'));
Dt = num2str(getappdata(0,'Dt'));
Lbc = num2str(getappdata(0,'Lbc'));
BC = num2str(getappdata(0,'BC'));
Tsi = num2str(getappdata(0,'Tsi'))
Tti = num2str(getappdata(0,'Tti'));
Tto = num2str(getappdata(0,'Tto'));
Tso = num2str(getappdata(0,'Tso'))
Ntt = num2str(getappdata(0,'Ntt'))
Ntp = num2str(getappdata(0,'Ntp'))
Nb = num2str(getappdata(0,'Nb'))
Theta_tp = num2str(getappdata(0,'Theta_tp'))
Qact=num2str(getappdata(0,'Qact'))
Uo=num2str(getappdata(0,'Uo'))
Delta_ps=num2str(getappdata(0,'Delta_ps'))
Material=num2str(getappdata(0,'Lambda_tw'));
alpha_s = num2str(getappdata(0,'alpha_s'))
alpha_t = num2str(getappdata(0,'alpha_t'))
effectiveness = num2str(getappdata(0,'effectiveness'))

currentholeid = moveToNextHole(doc)
append(doc,Ds,currentholeid);
currentholeid = moveToNextHole(doc)
append(doc,Dt,currentholeid);

currentholeid = moveToNextHole(doc)
append(doc,Material,currentholeid);

currentholeid = moveToNextHole(doc)
append(doc,Tti,currentholeid);
currentholeid = moveToNextHole(doc)
append(doc,Tto,currentholeid);

currentholeid = moveToNextHole(doc)
append(doc,Tsi,currentholeid);
currentholeid = moveToNextHole(doc)
append(doc,Tso,currentholeid);

currentholeid = moveToNextHole(doc)
append(doc,Nb,currentholeid);
currentholeid = moveToNextHole(doc)
append(doc,Lbc,currentholeid);


currentholeid = moveToNextHole(doc)
append(doc,Qact,currentholeid);
currentholeid = moveToNextHole(doc)
append(doc,Uo,currentholeid);

currentholeid = moveToNextHole(doc)
append(doc,Delta_ps,currentholeid);

currentholeid = moveToNextHole(doc)
append(doc,Ntt,currentholeid);
currentholeid = moveToNextHole(doc)
append(doc,Ntp,currentholeid);

currentholeid = moveToNextHole(doc)
append(doc,Theta_tp,currentholeid);

currentholeid = moveToNextHole(doc)
append(doc,alpha_t,currentholeid);

currentholeid = moveToNextHole(doc)
append(doc,alpha_s,currentholeid);

currentholeid = moveToNextHole(doc)
append(doc,effectiveness,currentholeid);

figure
p=plot(X1,Y1,X2,Y2,'--');

close(doc);
filename1 = strcat('C:\Users\SURAJ PATTEWAR\Documents\MATLAB\',name);
pdfFile = strcat(filename1,'.pdf');

[status,message] = doc2pdf('C:\Users\SURAJ PATTEWAR\Documents\MATLAB\Report.docx',pdfFile);
